...
*/
use sakila;
-- Q1. Rank films by length (filter out the rows with nulls or zeros in length column). Select only columns title, length and rank in your output.
...
-- Q6. Bonus: Which is the most rented film? (The answer is Bucket Brotherhood).
select count(rental_id) as 'Number of Rentals', title from rental 
join inventory i using(inventory_id)
join film using(film_id)
group by title
order by count(rental_id) desc
